...
import { SlackApproveLambda } from '../../lib/constructs/slack-approve-lambda.js';

describe('SlackApproveLambda Construct', () => {
  ...
  beforeAll(() => {
    ...
    new SlackApproveLambda(stack, 'TestSlackApprove', {
      isbApiBaseUrl: 'https://isb-api.example.com',
  ...
  describe('Environment Variables (AC6)', () => {
    it('includes ISB_API_BASE_URL', () => {
      template.hasResourceProperties('AWS::Lambda::Function', {
        Environment: {
          Variables: Match.objectLike({
            ISB_API_BASE_URL: 'https://isb-api.example.com',
          }),
        },
      });
    });

    it('includes ISB_JWT_SECRET_PATH', () => {
      template.hasResourceProperties('AWS::Lambda::Function', {
        Environment: {
          Variables: Match.objectLike({
            ISB_JWT_SECRET_PATH: '/approver/isb-jwt-secret',
          }),
        },
      });
    });
        ...
        PolicyDocument: {
          Statement: Match.arrayWith([
            Match.objectLike({
              ...
              Effect: 'Allow',
              Resource: Match.stringLikeRegexp('.*secret:/approver/isb-jwt-secret.*'),
            }),
          ]),
        },
      });
...
describe('SlackApproveLambda with default log level', () => {
  let template: Template;
